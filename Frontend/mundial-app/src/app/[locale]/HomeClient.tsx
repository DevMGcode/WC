...
export default function HomePage() {
  ...
  // Ya NO se redirige al anónimo a /login: la home es pública (anti-cloaking) y
  // el visitante sin sesión ve el bloque SSR editorial del server page (landing
  // con contenido + CTAs de login/registro). El dashboard solo se muestra con
  // sesión; el bloque SSR se oculta únicamente en ese caso para no duplicar.
  useEffect(() => {
    const ssr = document.getElementById('home-ssr-summary');
    if (!ssr) return;
    ssr.style.display = !authLoading && isAuthenticated ? 'none' : '';
